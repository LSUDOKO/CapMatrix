...
**CapMatrix** (formerly CLOVE) is an autonomous DeFi agent OS on Base mainnet. Users describe a strategy → AI compiles it into a visual workflow → agent executes it autonomously using a delegated USDC budget.
...
CAPMATRIX_INTERNAL_SECRET            # REQUIRED for internal server-to-server calls (intelligence/TTS/image); fallback CLOVE_INTERNAL_SECRET
NEXT_PUBLIC_CAPMATRIX_SESSION_ADDRESS # 1Shot wallet address (public); fallback NEXT_PUBLIC_CLOVE_SESSION_ADDRESS
