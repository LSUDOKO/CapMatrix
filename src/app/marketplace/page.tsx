...
export default function Marketplace() {
  ...
  return (
    <div className="min-h-screen bg-[#000] text-[#F0EDF5] font-sans">

      {/* Navbar */}
      <header className="sticky top-0 z-50 border-b border-[rgba(164,110,219,0.15)] bg-[rgba(6,10,8,0.9)] backdrop-blur-md">
        <nav className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-[0.78rem] font-medium text-[#D4C4EC]">
            ...
            <a href="/marketplace" className="text-[#A46EDB] border-b border-[#A46EDB] pb-0.5">Marketplace</a>
            <a href="#" className="hover:text-[#F0EDF5] transition-colors">Docs</a>
          </div>
          <a
            href="/dashboard"
      ...
      <main className="max-w-7xl mx-auto px-6 py-14 space-y-10">
        ...
        <div className="space-y-3 max-w-2xl">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-[#F0EDF5]">
            DeFi Agent{" "}
            <span
              className="bg-clip-text text-transparent"
              style={{ backgroundImage: "linear-gradient(90deg, #A46EDB, #22d3ee)" }}
            >
              Marketplace
            </span>
          </h1>
          <p className="text-sm leading-6 text-[#D4C4EC]">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2 flex-wrap">
            <Filter size={13} className="text-[#A46EDB]" />
            {FILTERS.map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`text-[11px] font-mono px-3 py-1.5 rounded-lg border transition-all ${
                  filter === f
                    ? "bg-[rgba(164,110,219,0.2)] border-[rgba(164,110,219,0.5)] text-[#A46EDB]"
                    : "border-[rgba(255,255,255,0.07)] text-[#8A7CB8] hover:text-[#D4C4EC] hover:border-[rgba(255,255,255,0.12)]"
                }`}
              >
                {f}
              </button>
          ...
          <div className="relative">
            <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A7CB8]" />
            <input
              type="text"
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search templates..."
              className="bg-[rgba(14,14,28,0.8)] border border-[rgba(164,110,219,0.2)] rounded-lg pl-8 pr-4 py-1.5 text-[11px] font-mono text-[#D4C4EC] placeholder-[#8A7CB8] focus:outline-none focus:border-[rgba(164,110,219,0.5)] w-52 transition-colors"
            />
          </div>
        </div>

        <div className="flex items-center justify-between border-b border-[rgba(164,110,219,0.1)] pb-4">
          <span className="text-[10px] font-mono text-[#8A7CB8]">
            Showing {filtered.length} verified strategy templates
          </span>
          <div className="flex items-center gap-2">
            <button className="text-[10px] font-mono px-3 py-1 rounded-lg bg-[rgba(164,110,219,0.15)] border border-[rgba(164,110,219,0.3)] text-[#A46EDB]">
              Newest
            </button>
            <button className="text-[10px] font-mono px-3 py-1 rounded-lg border border-[rgba(255,255,255,0.07)] text-[#8A7CB8] hover:text-[#D4C4EC] transition-colors">
              Top used
            </button>
          </div>
        </div>
          ...
          {filtered.map((strategy) => {
            ...
            return (
              <div
                ...
                className="group relative flex flex-col justify-between border border-[rgba(164,110,219,0.18)] bg-[#000] hover:border-[rgba(164,110,219,0.45)] rounded-xl p-6 cursor-pointer transition-all hover:shadow-[0_0_30px_rgba(164,110,219,0.1)] overflow-hidden"
              >
                {/* Top glow line on hover */}
                <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[rgba(164,110,219,0.5)] to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />

                <div className="space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h3 className="text-[14px] font-bold text-[#F0EDF5] group-hover:text-[#A46EDB] transition-colors leading-snug">
                          {strategy.name}
                        </h3>
                      </div>
                      <p className="text-[10px] font-mono text-[#8A7CB8]">
                        by <span className="text-[#D4C4EC]">{strategy.creator}</span>
                      </p>
                    </div>
                    <span
                  ...
                  <p className="text-[12px] leading-5 text-[#D4C4EC]">{strategy.description}</p>

                  <div className="flex flex-wrap gap-1.5">
                    {strategy.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-[9px] font-mono text-[#8A7CB8] border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] px-2 py-0.5 rounded"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="flex items-center justify-between border-t border-[rgba(164,110,219,0.1)] mt-5 pt-4">
                  <div className="flex items-center gap-3 text-[10px] font-mono text-[#8A7CB8]">
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg border transition-colors ${
                        isLiked
                          ...
                          : "text-[#8A7CB8] border-[rgba(255,255,255,0.06)] hover:border-[rgba(232,121,249,0.2)]"
                      }`}
                    >
                      <Heart size={12} fill={isLiked ? "currentColor" : "none"} />
                    </button>
                    <button
                      className="text-[10px] font-bold font-mono px-3 py-1.5 rounded-lg bg-[rgba(164,110,219,0.15)] border border-[rgba(164,110,219,0.3)] text-[#A46EDB] hover:bg-[rgba(164,110,219,0.04)] flex items-center gap-1 transition-all group-hover:bg-[#A46EDB] group-hover:text-white"
                    >
