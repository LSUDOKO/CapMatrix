...
/**
 * Strategy-art image (report visual) — powered by Venice AI (flux-2-max).
 *
 * Internal endpoint called by the agent runtime. Uses the same VENICE_API_KEY.
 * On failure returns a branded SVG fallback so the report always has a visual.
 */
export async function POST(request: NextRequest) {
  ...
  if (!veniceKey) {
    return NextResponse.json({ imageUrl: generateFallbackSvgDataUrl(ctx), fallback: true, reason: "VENICE_API_KEY not set" });
  }
  ...
  try {
    const res = await fetch("https://api.venice.ai/api/v1/image/generate", {
      ...
      body: JSON.stringify({
        model: "flux-2-max", prompt, width: 1024, height: 576, steps: 4, cfg_scale: 7.5,
    ...
    if (!res.ok) {
      ...
      console.warn("[media/image] Venice image failed:", res.status, errText.slice(0, 200));
      return NextResponse.json({ imageUrl: generateFallbackSvgDataUrl(ctx), fallback: true, reason: `Venice ${res.status}` });
    }
    const data = await res.json() as { images?: string[]; id?: string };
    const b64  = data.images?.[0];
    if (!b64) throw new Error("No image in Venice response");
    return NextResponse.json({ imageUrl: `data:image/webp;base64,${b64}`, width: 1024, height: 576, prompt });
  } catch (e) {
    console.warn("[media/image] Venice exception:", e);
    return NextResponse.json({ imageUrl: generateFallbackSvgDataUrl(ctx), fallback: true, reason: e instanceof Error ? e.message : String(e) });
  }
}

// ── Helpers ────────────────────────────────────────────────────────────────────

function buildImagePrompt(raw?: string, ctx?: { apy?: number; protocol?: string; action?: string }): string {
  const palette = "quiet luxury, paper cream #1A0033, ink #0B0C09, single acid lime #A46EDB accent";
...
function generateFallbackSvgDataUrl(ctx: { protocol?: string; apy?: number; action?: string }): string {
  ...
  <defs><radialGradient id="g" cx="50%" cy="40%" r="60%">
    <stop offset="0%" stop-color="#A46EDB" stop-opacity="0.55"/><stop offset="60%" stop-color="#A46EDB" stop-opacity="0"/>
  </radialGradient></defs>
  ...
  <circle cx="900" cy="80" r="6" fill="#A46EDB"/><circle cx="930" cy="80" r="6" fill="#A46EDB" opacity="0.6"/><circle cx="960" cy="80" r="6" fill="#A46EDB" opacity="0.3"/>
</svg>`;
