      ...
      try {
        ...
        if (!agent.registryId && process.env.QUICKNODE_ENDPOINT) {
          const reg = await registerAgentOnChain({
            agentAddress: process.env.NEXT_PUBLIC_CAPMATRIX_SESSION_ADDRESS ?? process.env.NEXT_PUBLIC_CLOVE_SESSION_ADDRESS ?? agent.walletAddress,
            name:         agent.name,
            goal:         agent.goal,
          });
...
/**
 * Some Venice/llama models emit tool calls as TEXT in the message content
 * (e.g. `<function=executeDefi,{"protocol":"lido",...}>` or
 * `<function=executeDefi>{...}</function>`) instead of the structured
 * `tool_calls` field. Left unparsed, the call is shown as a "thinking" node and
 * never executed — so no transaction happens and the model then hallucinates a
 * fake result. Parse those out so they actually run.
 */
function parseTextToolCalls(content: string): { name: string; args: Record<string, unknown> }[] {
  ...
  const re = /<function\s*=\s*([a-zA-Z_]\w*)\s*[,>]?\s*(\{[\s\S]*?\})/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(content)) !== null) {
    try {
      out.push({ name: m[1], args: JSON.parse(m[2]) as Record<string, unknown> });
...
If notifyUser is one of your tools, its "message" argument MUST be a finished
report written from the REAL data returned by the tool calls you already made
in this subgoal (actual APY numbers, amounts, protocol names, hold/rebalance
decision). NEVER pass the subgoal description back as the message, and NEVER
output placeholder text like "insert summary here" or "<result>" — if you
don't have a concrete number yet, write what you actually know instead.
...
Once the subgoal is done, output a one-line summary as plain text (no tool call).`,
  ...
  for (let iter = 0; iter < 3; iter++) {
    ...
    const rawThinking = typeof msg.content === "string" ? msg.content.trim() : "";

    // Preferred path: structured tool_calls.
    const structuredCalls = (msg.tool_calls ?? []).map(tc => {
      const c = tc as unknown as { id: string; function: { name: string; arguments: string } };
      let a: Record<string, unknown> = {};
      try { a = JSON.parse(c.function.arguments) as Record<string, unknown>; } catch { /**/ }
      return { id: c.id, name: c.function.name, args: a, structured: true as const };
    });

    // Fallback: the model wrote the tool call as text inside the content
    // (`<function=executeDefi,{…}>`) instead of using tool_calls. Without this
    // the action would never execute and the agent would hallucinate a result.
    const textCalls = structuredCalls.length
      ? []
      : parseTextToolCalls(rawThinking).map((p, i) => ({
          id: `text_${iter}_${i}`, name: p.name, args: p.args, structured: false as const,
        }));

    // LIVE THINKING — surface reasoning, but strip any <function=…> blobs so the
    // canvas shows clean thinking, not raw tool-call syntax.
    const cleanThinking = rawThinking.replace(/<function[\s\S]*?(?:>|<\/function>)/g, "").trim();
    if (cleanThinking) {
      await emit("plan", { observation: cleanThinking.slice(0, 320) }, parentId, { x: cx - 220, y: cy });
      cy += 80;
    }

    const calls = structuredCalls.length ? structuredCalls : textCalls;
    ...
    for (const call of calls) {
      ...
      const isMetaTool = name === "addThought" || name === "revisePlan";
      ...
      if (isMetaTool) {
        ...
        results.push(result);
        if (name === "addThought" && typeof args.text === "string" && args.text.trim()) {
          await emit("plan", { observation: args.text }, parentId, { x: cx, y: cy });
          cy += 80;
        }
        if (call.structured) toolResults.push({ role: "tool", tool_call_id: call.id, content: result.result });
        else textNotes.push(`${name} → ${result.result}`);
        continue;
      }

      // Emit tool-call thought BEFORE execution so the canvas shows the action.
      const callId = await emit("tool-call", { tool: name, args }, parentId, { x: cx, y: cy });
      cy += 90;
    ...
    // Text-format calls have no matching tool_call id on the assistant message,
    // so we can't reply with role:"tool" — feed the REAL results back as a user
    // turn so the model continues from facts, not invention.
    if (textNotes.length) {
      messages.push({
        role: "user",
        content: `Tool results:\n${textNotes.join("\n")}\n\nContinue using these REAL results. Never invent numbers, tx hashes, or amounts.`,
      });
    }
  }
  return results;
