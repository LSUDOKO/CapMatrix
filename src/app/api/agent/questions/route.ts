...
import { inferAgentType, listAgentTypes } from "@/lib/agent/agentTypes";
...
- "liquid-staking" — Stakes idle USDC into wstETH for ETH staking yield (Base). Pick this when the prompt mentions staking, liquid staking, Lido, stETH, or wstETH.
  ...
  "questions": [  /* ONLY the still-missing fields; [] if none */
    {
      ...
      "type": "single",
      "options": ["yield", "copy-trader", "rebalancer", "liquid-staking"]
    },
    {
      "id": "protocols",
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let questions = Array.isArray(parsed.questions) ? parsed.questions : defaultQuestions();
    const prefilled: Record<string, unknown> = { ...(parsed.prefilled ?? {}) };

    // ── Make the questionnaire smart: don't ask what the agent type already
    // decides. The LLM is unreliable about skipping these, so enforce it here. ──
    const rawType = String(prefilled.agentType ?? "").toLowerCase().trim();
    const agentType = rawType || inferAgentType(prompt);

    // Liquid-staking always uses Lido (USDC → wstETH). Don't ask the protocol.
    if (agentType === "liquid-staking") {
      prefilled.protocols = ["Lido"];
      questions = questions.filter(q => q.id !== "protocols");
    }
    // Liquid-staking and rebalancer are single-agent by design — never a team.
    if (agentType === "liquid-staking" || agentType === "rebalancer") {
      prefilled.orchestration = "Single agent (simple)";
      questions = questions.filter(q => q.id !== "orchestration");
    }
    // Copy-trader picks tokens from on-chain whale activity, not a lending
    // protocol — the protocols question is pure noise. It defaults to a single
    // agent; only keep the architecture question when the prompt asks for a team.
    if (agentType === "copy-trader") {
      questions = questions.filter(q => q.id !== "protocols");
      const wantsTeam = /(multi|team|fund manager|aggressive and conservative|swarm|several agents|two agents)/i.test(prompt);
      if (!wantsTeam) {
        prefilled.orchestration = prefilled.orchestration ?? "Single agent (simple)";
        questions = questions.filter(q => q.id !== "orchestration");
      }
    }
  ...
  return [
    {
      ...
      hint: "Each archetype perceives a different real data source and acts on its own",
      type: "single",
      // Derived from the registry so a new archetype shows up automatically.
      options: listAgentTypes().map(t => t.type),
    },
    {
