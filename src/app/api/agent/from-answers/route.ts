...
import { getAgentTypeDef, buildTypeSystemPrompt, inferAgentType, listAgentTypes, COPY_TIER_LIQUIDITY_BOUNDARY_USD } from "@/lib/agent/agentTypes";
...
export async function POST(request: NextRequest) {
  ...
  // Derived from the registry so new archetypes are accepted automatically.
  const VALID_TYPES: AgentType[] = listAgentTypes().map(t => t.type);
  ...
  // ── Copy-trade PIPELINE detection ──────────────────────────────────────────
  // "2 copiers → invest the profit into yield/staking → rebalance if it goes bad"
  // is a HETEROGENEOUS team: copy desk + a profit allocator + a risk sentinel.
  // The questionnaire often sets agentType to the ALLOCATOR type (yield/liquid-
  // staking), which would wrongly build ONE agent — force the copy team instead.
  const mentionsCopy   = /copy.?trad|mirror|whale|smart.?money|follow .*wallet/i.test(prompt);
  const wantsAllocator = /(invest|deposit|put|allocate|compound|reinvest)[^.]*?(profit|gain|yield|stak)|profit[^.]*?(yield|stak|invest|deposit|liquid)|liquid.?stak|wsteth/i.test(prompt);
  const wantsSentinel  = /rebalanc|sentinel|protect|stop.?loss|drawdown|if[^.]*?(bad|drop|down|crash|loss|wrong|risk)/i.test(prompt);
  const isCopyPipeline = mentionsCopy && (wantsAllocator || wantsSentinel);
  const allocatorType: AgentType = /liquid.?stak|wsteth|steth|lido/i.test(prompt) ? "liquid-staking" : "yield";
  if (isCopyPipeline) agentType = "copy-trader";
  ...
  const isMulti      = (wantsMulti && TEAM_CAPABLE.includes(agentType)) || isCopyPipeline;
  ...
  // "Aggressive — always seek best yield", not just "Aggressive". For multi-agent
  // teams `risk` can be an ARRAY (one per worker), so normalize to a string first.
  const riskStr   = Array.isArray(risk) ? risk.join(" ") : String(risk ?? "");
  ...
  const scheduleStr    = Array.isArray(schedule) ? String(schedule[0] ?? "Daily") : String(schedule ?? "Daily");
  const scheduleClause = scheduleStr === "Every hour" ? "Run every hour." : `Run ${scheduleStr.toLowerCase()}.`;
  ...
  const scheduleIntervalMs: number | undefined =
      scheduleStr === "Every minute"    ? 60 * 1000
    : scheduleStr === "Every 5 minutes" ? 5   * 60 * 1000
    : scheduleStr === "Every hour"      ? 60  * 60 * 1000
    : scheduleStr === "Every 6 hours"   ? 6   * 60 * 60 * 1000
    : scheduleStr === "Daily"           ? 24  * 60 * 60 * 1000
    : scheduleStr === "Weekly"          ? 7   * 24 * 60 * 60 * 1000
    : scheduleStr === "On-demand only"  ? undefined
  ...
  if (isMulti && agentType === "copy-trader") {
    ...
    // Pipeline reserves budget slices for the profit allocator + risk sentinel;
    // a plain copy desk keeps the original 70/30 split (copyPct = 1).
    const hasAlloc  = isCopyPipeline && wantsAllocator;
    const hasSent   = isCopyPipeline && wantsSentinel;
    const allocPct  = hasAlloc ? 0.20 : 0;
    const sentPct   = hasSent  ? 0.10 : 0;
    const copyPct   = 1 - allocPct - sentPct;
    const consCap   = Number((budgetNum * copyPct * 0.7).toFixed(4));
    const aggrCap   = Number((budgetNum * copyPct - consCap).toFixed(4));
    const allocCap  = Number((budgetNum * allocPct).toFixed(4));
    const sentCap   = Number((budgetNum * sentPct).toFixed(4));
    ...
    const workerContext = async (childId: string, cap: number, protocols: string[] = tierProtocols): Promise<{ context: string; hash: string }> => {
      if (isFundManagerGrant) {
        try {
          const { buildRedeemableWorkerChain } = await import("@/lib/web3/subDelegation");
          const chain = await buildRedeemableWorkerChain(effectivePermContext!, childId, protocols, cap, typeDef.chainId);
          return { context: chain.context, hash: chain.scopedHash };
        } catch (e) {
          console.warn("[from-answers] worker chain build failed — root fallback:", e instanceof Error ? e.message : e);
        }
      }
      return { context: effectivePermContext!, hash: "0xpending" };
    };
    ...
    for (let i = 0; i < tierDefs.length; i++) {
      ...
      const ctx = await workerContext(copier.id, t.cap);
    ...
    // ── PIPELINE EXTRAS — heterogeneous team members ──────────────────────────
    // They coordinate with the copy desk through SHARED on-chain state (the
    // wallet's positions + idle USDC), each with its own on-chain-capped slice.
    const extras: Awaited<ReturnType<typeof createAgent>>[] = [];

    // Profit Allocator — reinvests the team's idle / realized USDC into yield/staking.
    if (hasAlloc) {
      const allocProtocols = allocatorType === "liquid-staking" ? ["lido", "uniswap"] : ["morpho", "aave"];
      const allocator = await createAgent({
        walletAddress,
        name: allocatorType === "liquid-staking" ? "Staking Allocator" : "Yield Allocator",
        goal:
          `You are the copy desk's PROFIT ALLOCATOR. After the copiers trade, the team holds positions and ` +
          `sometimes idle USDC from realized gains. Each run: read the team's on-chain positions and idle USDC ` +
          `balance, and when there is idle/realized USDC sitting uninvested, ` +
          `${allocatorType === "liquid-staking" ? "stake it into wstETH via Lido" : "deposit it into the best safe yield (Morpho/Aave)"} ` +
          `so profit compounds instead of sitting idle. Only deploy what is genuinely idle — never force a deposit. ` +
          `Stay within your on-chain cap of ${allocCap} USDC; overspend reverts. Report what you reinvested.`,
        budgetUsdc: String(allocCap),
        scheduleIntervalMs,
        mediaPolicy: "off",
        position: { x: 860, y: 40 },
        workflowId: workflow.id,
        agentType: allocatorType,
        typeConfig: { role: "profit-allocator", teamWorkflowId: workflow.id },
      });
      await addAgentToWorkflow(workflow.id, allocator.id);
      const ctx = await workerContext(allocator.id, allocCap, allocProtocols);
      await setDelegation(allocator.id, {
        parentAgentId:            fundManager.id,
        delegationContext:        ctx.context,
        delegationHash:           ctx.hash,
        delegationManagerAddress: effectiveDelegationManager!,
        delegationCap:            String(allocCap),
      });
      if (ctx.context !== effectivePermContext) await updateAgent(allocator.id, { rootFallbackContext: effectivePermContext });
      extras.push(allocator);
    }

    // Rebalancer Sentinel — watches the team's positions, protects on drawdown/risk.
    if (hasSent) {
      const sentProtocols = ["morpho", "aave", "lido", "uniswap", "aerodrome"];
      const sentinel = await createAgent({
        walletAddress,
        name: "Rebalancer Sentinel",
        goal:
          `You are the team's RISK SENTINEL. Each run: read the team's on-chain positions (monitorPositions) and ` +
          `check protocol risk (checkRisk). If any position has drawn down more than 15% from entry, OR a protocol ` +
          `shows HIGH risk (exploit, depeg, pause), rebalance those funds into a safer protocol (or back to USDC) and ` +
          `alert the user. Only act on a real threshold breach — otherwise HOLD and report all-clear. Stay within ` +
          `your on-chain cap of ${sentCap} USDC; overspend reverts.`,
        budgetUsdc: String(sentCap),
        scheduleIntervalMs,
        mediaPolicy: "off",
        position: { x: 860, y: 280 },
        workflowId: workflow.id,
        agentType: "rebalancer",
        typeConfig: { role: "sentinel", drawdownThresholdPct: 15, teamWorkflowId: workflow.id },
      });
      await addAgentToWorkflow(workflow.id, sentinel.id);
      const ctx = await workerContext(sentinel.id, sentCap, sentProtocols);
      await setDelegation(sentinel.id, {
        parentAgentId:            fundManager.id,
        delegationContext:        ctx.context,
        delegationHash:           ctx.hash,
        delegationManagerAddress: effectiveDelegationManager!,
        delegationCap:            String(sentCap),
      });
      if (ctx.context !== effectivePermContext) await updateAgent(sentinel.id, { rootFallbackContext: effectivePermContext });
      extras.push(sentinel);
    }

    const chainSummary =
      `Fund Manager → Conservative + Aggressive Copier` +
      (hasAlloc ? ` → ${allocatorType === "liquid-staking" ? "Staking" : "Yield"} Allocator` : "") +
      (hasSent  ? ` + Rebalancer Sentinel` : "");

    return NextResponse.json({
      agents: [fundManager, ...copiers, ...extras],
      workflow, wired: true,
      chain: chainSummary,
    });
  }
