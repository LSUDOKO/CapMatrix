...
export async function GET(request: NextRequest) {
  ...
  if (effectivePositions.length === 0 && runs.length > 0) {
    ...
    for (const r of runs) {
      // Skip holds and — crucially — any run with NO txHash. A recorded run
      // without a tx never moved funds on-chain (failed, relayer-dropped, or
      // hallucinated); counting it inflates "claimed" and creates phantom drift.
      if (!r.success || r.action === "hold" || !r.txHash) continue;
      const amt = Number.parseFloat(r.amount) || 0;
      if (amt <= 0) continue;
      // Net withdrawals/exits OUT so the same capital deposited→withdrawn→
      // redeposited across runs isn't double-counted as fresh deployed capital.
      const isWithdraw = /withdraw|exit|redeem|unstake/i.test(r.action);
      const cur = byProto.get(r.protocol) ?? { amount: 0, apy: 0, n: 0 };
      cur.amount += isWithdraw ? -amt : amt;
      if (!isWithdraw) { cur.apy += r.apy || 0; cur.n += 1; }
      byProto.set(r.protocol, cur);
    }
    effectivePositions = [...byProto.entries()]
      .filter(([, v]) => v.amount > 0.0001)   // drop fully-exited protocols
      .map(([protocol, v]) => ({
        walletAddress: wallet, protocol, amount: String(+v.amount.toFixed(6)),
        entryApy: v.n ? +(v.apy / v.n).toFixed(2) : 0,
        entryTimestamp: new Date(), updatedAt: new Date(),
      }));
  }
  let deployedUsd = effectivePositions.reduce((s, p) => s + (Number.parseFloat(p.amount) || 0), 0);
  ...
  // Reconcile DISPLAYED lending positions to their REAL on-chain balance so the
  // dashboard shows truth, not a stale DB claim. Conservative: only override
  // when we got a confident non-zero read (a 0 may be an RPC hiccup, not an
  // empty position). The `audit` array above still reports claimed-vs-actual,
  // so any DB lag stays visible and explained rather than silently hidden.
  const actualByProto = new Map(audit.map(a => [a.protocol.toLowerCase(), a.actualUsdc]));
  effectivePositions = effectivePositions.map(p => {
    const actual = actualByProto.get(p.protocol.toLowerCase());
    return actual !== undefined && actual > 0 ? { ...p, amount: String(actual) } : p;
  });
  deployedUsd = effectivePositions.reduce((s, p) => s + (Number.parseFloat(p.amount) || 0), 0);
