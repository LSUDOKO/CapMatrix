...
function TokenLogo({ address, size = 16 }: { address?: string; size?: number }) {
  ...
  return (
    ...
    <img
      ...
      style={{ borderRadius: "50%", flexShrink: 0, background: "rgba(20,21,16,0.08)" }}
    />
  );
}
...
export function AgentThoughtNode({ data, selected }: { data: ThoughtNodeData; selected?: boolean }) {
  ...
  return (
    <div
      ...
      style={{
        position: "relative",
        background: type === "goal" ? `linear-gradient(135deg, rgba(164,110,219,0.06), transparent 70%), ${CARD}` : CARD,
        ...
        cursor: "pointer",
        boxShadow: selected
          ? `0 0 0 1px ${ACCENT}, 0 14px 30px -18px rgba(164,110,219,0.45)`
...
function handleStyle(selected?: boolean): React.CSSProperties {
  return { background: selected ? ACCENT : "rgba(20,21,16,0.25)", width: 7, height: 7, border: "2px solid #ECE8DE" };
}
        ...
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
          {subgoals.slice(0, 5).map((sg, i) => (
            <div key={sg.id} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              <span style={{ flexShrink: 0, width: 16, height: 16, borderRadius: 5, background: "rgba(164,110,219,0.12)", color: ACCENT, fontSize: 9.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 1 }}>{i + 1}</span>
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {(submitted || txHash) && (
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 9px", borderRadius: 7, background: "rgba(164,110,219,0.08)", border: "1px solid rgba(164,110,219,0.22)" }}>
            <IconCheck size={12} />
