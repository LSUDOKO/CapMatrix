...
/** Per-chain relayer config. Supports Sepolia testnet. */
const RELAYER_CHAINS: Record<number, { target: `0x${string}`; usdc: `0x${string}` }> = {
  [SEPOLIA_CHAIN_ID]: {
    target: "0x26a529124f0bbf9af9d8f9f84a43efe47cf1199a", // same target on all chains
    usdc:   TOKENS.USDC[CHAIN.SEPOLIA] as `0x${string}`,
  },
};

const RELAYER_TARGET = RELAYER_CHAINS[SEPOLIA_CHAIN_ID].target;
const USDC           = RELAYER_CHAINS[SEPOLIA_CHAIN_ID].usdc;
...
/** Fetch fresh fee quote for the relayer's USDC on the given chain (default Sepolia). */
export async function getRelayerFeeData(chainId: number = SEPOLIA_CHAIN_ID): Promise<RelayerFeeData> {
  const cfg = RELAYER_CHAINS[chainId] ?? RELAYER_CHAINS[SEPOLIA_CHAIN_ID];
...
export async function executeViaPublicRelayer(
  ...
  const chainCfg = RELAYER_CHAINS[chainId] ?? RELAYER_CHAINS[SEPOLIA_CHAIN_ID];
