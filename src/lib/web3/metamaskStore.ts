...
// Persists across reloads — `eth_accounts` still returns the address after
// disconnect() (MetaMask keeps the site authorized), so init() needs an
// explicit "user chose to disconnect" flag to avoid silently reconnecting.
const DISCONNECT_FLAG = "clove_wallet_disconnected";
...
class MetaMaskStore {
  ...
  async init() {
    if (typeof window !== "undefined" && window.localStorage.getItem(DISCONNECT_FLAG)) {
      return;
    }
  ...
  async connect() {
    ...
    try {
      ...
      if (typeof window !== "undefined") window.localStorage.removeItem(DISCONNECT_FLAG);
  ...
  async revokeOnChain(): Promise<RevocationResult | null> {
    ...
    try {
      ...
      // A local-only / demo permission has nothing on-chain to revoke — clear it
      // locally instead of surfacing a scary error (and never opening MetaMask).
      if (msg.includes("LOCAL_ONLY")) {
        await this.deletePermissionFromDb();
        this.state = { ...this.state, permission: null, isRevoking: false };
        this.notify();
        this.log("success", "Permission cleared (it was local-only — nothing was on-chain to revoke).");
        return null;
      }
      this.state = { ...this.state, isRevoking: false };
      this.notify();
      this.log("error", `Revocation failed: ${msg}`);
      return null;
    }
  }
  ...
  disconnect() {
    if (typeof window !== "undefined") window.localStorage.setItem(DISCONNECT_FLAG, "1");
    // Actually revoke the site's eth_accounts permission so MetaMask stops
    // re-reporting the address after a refresh (best-effort, fire-and-forget).
  ...
  /**
   * Called by PrivyProvider after Privy auth. Overrides MetaMask-only flow.
   * data=null means the user logged out of Privy.
   */
  applyPrivySession(data: PrivySession | null) {
    if (!data) {
      this.state = {
        mode: "disconnected",
        userAddress: null,
    ...
    this.notify();
    this.log("success", `Privy connected: ${data.address}`);
    void Promise.all([
      this.fetchSessionAddress(),
      this.fetchPermissionFromDb(data.address),
    ]);
  }
}
