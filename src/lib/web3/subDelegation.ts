...
import { MORPHO, AAVE_V3, UNISWAP_V3, TOKENS, CHAIN } from "@/lib/protocols/addresses";

const SEPOLIA_CHAIN_ID = 11155111;

/** USDC on Sepolia */
const USDC = TOKENS.USDC[CHAIN.SEPOLIA] as `0x${string}`;

export const PROTOCOL_TARGETS: Record<string, `0x${string}`[]> = {
  aave:      [AAVE_V3.pool[CHAIN.SEPOLIA] as `0x${string}`],
  uniswap:   [(UNISWAP_V3.swapRouter[CHAIN.SEPOLIA]) as `0x${string}`],
};
