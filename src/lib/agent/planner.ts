...
3. When calling rebalance, "amount" MUST be the size of the EXISTING position in fromProtocol — copy the "amount" field for that protocol straight from monitorPositions' result. NEVER pass the agent's total budget; that will trip the 95% budget guard and the rebalance will be blocked.
...
export function imagePromptForReflection(
  ...
  const palette = "quiet luxury palette, paper #1A0033 background, ink #000, single jolt of acid lime #A46EDB";
