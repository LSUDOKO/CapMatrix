...
export async function createAgent(input: {
  ...
  try {
    ...
    onChainAddress = process.env.NEXT_PUBLIC_CAPMATRIX_SESSION_ADDRESS ?? process.env.NEXT_PUBLIC_CLOVE_SESSION_ADDRESS ?? undefined;
  }
...
export async function agentOnChainAddress(agent: Agent): Promise<`0x${string}`> {
  ...
  try {
    ...
    return (process.env.NEXT_PUBLIC_CAPMATRIX_SESSION_ADDRESS ?? process.env.NEXT_PUBLIC_CLOVE_SESSION_ADDRESS ?? "0x") as `0x${string}`;
  }
}
