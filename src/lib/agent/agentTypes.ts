...
export const AGENT_TYPES: Record<AgentType, AgentTypeDef> = {
  ...
  yield: {
    ...
    dataSources: ["DeFiLlama Yields API", "Venice web search"],
    defaultBudget: "10",
...
Think in portfolio terms, not just chasing the single highest APY. Only rebalance when the improvement clearly beats the switching cost.`,
  ...
  // ── 5. Liquid staking agent (Base) ───────────────────────────────────────────
  "liquid-staking": {
    type:        "liquid-staking",
    label:       "Liquid Staking Agent",
    emoji:       "🥩",
    tagline:     "Stakes idle USDC into wstETH so it earns ETH staking yield",
    chainId:     CHAIN.SEPOLIA,
    chainName:   "Base",
    // executeDefi with protocol "lido" / action "stake" routes through the
    // lido-wrap method (USDC → wstETH swap on Base) in /api/execute/defi.
    tools:       ["checkYields", "checkRisk", "executeDefi", "notifyUser", "addThought"],
    dataSources: ["DeFiLlama Yields API", "Venice web search"],
    defaultBudget: "10",
    defaultIntervalMs: DAY,
    systemPrompt:
      `You are {name}, an autonomous liquid-staking agent on Base. Budget: {budget} USDC.
Config: {config}

Your job is to convert idle USDC into a liquid staking position (wstETH) so it earns ETH
staking yield while staying liquid and zero-custody — the wstETH lands in the user's own wallet.

Workflow:
1. Call checkYields (and optionally checkRisk) to confirm staking conditions are healthy — no
   recent depeg, exploit, or stETH/ETH dislocation. Stake only when it's safe.
2. When healthy, call executeDefi with protocol "lido" and action "stake" to acquire wstETH.
   "amount" should be about 90% of your budget — leave ~10% for the relayer's USDC
   gas fee (e.g. with a 10 USDC budget, stake 9). Never request the full budget.
3. notifyUser with the amount staked, the live staking APR you saw, and the resulting position.

You are NOT a yield-hopper. Stake into wstETH and hold; only act again if Config changes or a
risk event makes holding unsafe. Report honestly with real numbers — never use placeholder text.`,
  },
};
...
export function inferAgentType(prompt: string): AgentType {
  ...
  if (/(liquid stak|wsteth|wrapped steth|steth|lido|stake .*(eth|usdc)|staking yield)/.test(p)) return "liquid-staking";
