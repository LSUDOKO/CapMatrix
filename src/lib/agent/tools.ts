    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          amount:       { type: "string", description: "USDC size of the EXISTING position in fromProtocol (from monitorPositions' `amount` field) — NOT the agent's total budget. Moving more than you actually hold in fromProtocol will fail or trip the budget guard." },
...
Return ONLY JSON: { "riskLevel": "LOW"|"MEDIUM"|"HIGH", "safeToExecute": true|false, "reason": "<1 sentence>" }`,
    ...
    // First neutralize NEGATED / "check-for" mentions — the agent's own risk
    // instructions say things like "no recent exploits" or "look for depeg",
    // and matching those raw would spuriously flag HIGH on safe blue-chip
    // protocols (e.g. Lido), blocking every execution.
    const text = context
      .toLowerCase()
      .replace(
        /\b(no|not|without|any|zero|free of|look(?:ing)? for|check(?:ing)? for|search(?:ing)? for|watch(?:ing)? for|monitor(?:ing)? for|scan(?:ning)? for|confirm)\b[\s\w,'-]{0,40}?\b(hack\w*|exploit\w*|vulnerab\w*|attack\w*|breach\w*|drain\w*|rug\w*|pause\w*|depeg\w*|dislocat\w*|risk\w*|volatil\w*|warning|caution|uncertainty|dip)\b/g,
        " ",
      );
  ...
  if (name === "executeDefi") {
    ...
    const requestedAmount = String(args.amount ?? "0.1");

    // â”€â”€ BUDGET GUARD â€” CLAMP to 95% of remaining budget (leaves USDC for the
    // relayer gas fee). Previously this hard-blocked, so an agent that asked to
    // deposit its whole budget did NOTHING. Clamping means a real (slightly
    // smaller) transaction still executes — far better UX, reported honestly. â”€â”€
    const amountNum = Number.parseFloat(requestedAmount) || 0;
    const budgetNum = Number.parseFloat(ctx.budgetUsdc) || 0;
    const used      = ctx.budgetUsedUsdc ?? 0;
    const maxSpend  = budgetNum > 0 ? Math.max(0, budgetNum * 0.95 - used) : amountNum;
    let amount      = requestedAmount;
    let clamped     = false;
    if (budgetNum > 0 && amountNum > maxSpend) {
      if (maxSpend < 0.01) {
        return {
          tool: name,
          args,
          result: JSON.stringify({
            blocked:    true,
            reason:     "No spendable budget left (95% cap reached after fee headroom)",
      ...
      amount  = String(Math.floor(maxSpend * 1e6) / 1e6); // floor to USDC atoms
      clamped = true;
    }
    ...
    try {
      ...
      return {
        ...
        result: JSON.stringify({
          ...
          protocol,
          amount,
          ...(clamped ? { clamped: true, requestedAmount, note: `Amount reduced from ${requestedAmount} to ${amount} USDC to stay under the 95% budget cap and leave USDC for the relayer fee.` } : {}),
